...
{
	ExtraContainerChanges::ExtendDataList* xData = MemoryManager::Allocate<ExtraContainerChanges::ExtendDataList>();
	if (xData) {
		memset(xData, 0, sizeof(ExtraContainerChanges::ExtendDataList));
		if (pExtraDataList)
			if (xData->AddAt(pExtraDataList, eListEnd)==eListInvalid) {
				MemoryManager::Deallocate(xData);
				xData = NULL;
			}
	}
	return xData;
	...
	if (xData && xList) {
		...
		if (pExtraDataList == xList) {
			...
			if (bFreeList) {
				for (UInt32 j = 0 ; j<0xFF ; j++)
					pExtraDataList->RemoveByType(j);
				MemoryManager::Deallocate(xList);
			}
			return true;
		}
	}
...
{
	ExtraContainerChanges::EntryData* xData = MemoryManager::Allocate<ExtraContainerChanges::EntryData>();
	if (xData) {
	...
	if (extendData && toRemove) {
		...
		if (index >= 0) {
			...
			if (bFree) {
				...
				MemoryManager::Deallocate(toRemove);
			}
			return true;
		}
...
{
#ifdef RUNTIME
	ExtraContainerChanges::EntryDataList* xData = MemoryManager::Allocate<ExtraContainerChanges::EntryDataList>();
	if (xData) {
...
{
	Data* data = MemoryManager::Allocate<Data>();
	if (data) {
		data->owner = owner;
		data->objList = NULL;
		data->unk2 = 0.0;
...
void ExtraContainerChangesFree(ExtraContainerChanges* xData, bool bFreeList) {
	if (xData) {
		if (xData->data) {
			...
			MemoryManager::Deallocate(xData->data);
		}
		MemoryManager::Deallocate(xData);
	}
}
...
void ExtraContainerChanges::Cleanup()
{
	if (data && data->objList) {
		for (EntryDataList::Iterator iter = data->objList->Begin(); !iter.End(); ++iter) {
			...
			do
			{
				ExtraDataList* xtendData = xIter->m_item;
				if (xtendData && !xtendData->m_data)
				{
					MemoryManager::Deallocate(xtendData);
					xIter = xIter->RemoveMe();
					if (!xIter) break;
				}

			} while (xIter = xIter->m_pkNext);
		}
	}
}
...
{
	ExtraContainerChanges* xChanges = (ExtraContainerChanges*)refr->ExtraList.GetByType(kExtraData_ContainerChanges);
	if (!xChanges) {
		xChanges = ExtraContainerChanges::Create();
		refr->ExtraList.Add(xChanges);
	}
	return xChanges;
}
...
{
	void* memory = MemoryManager::Allocate(size);
...
ExtraLock* ExtraLock::Create()
{
	...
	ExtraLock::Data* lockData = MemoryManager::Allocate<ExtraLock::Data>();
...
ExtraTeleport* ExtraTeleport::Create()
{
	...
	// create data
	ExtraTeleport::Data* data = MemoryManager::Allocate<ExtraTeleport::Data>();
...
void ExtraScript::EventCreate(UInt32 eventCode, TESObjectREFR* container) {
	if (eventList) {
		// create Event struct
		ScriptEventList::Event * pEvent = MemoryManager::Allocate<ScriptEventList::Event>();
		if (pEvent) {
		...
		if (!eventList->m_eventList) {
			eventList->m_eventList = MemoryManager::Allocate<ScriptEventList::EventList>();
...
ExtraFactionChanges* ExtraFactionChanges::Create()
{
	...
	ExtraFactionChanges::FactionListEntry* FactionChangesData = MemoryManager::Allocate<FactionListEntry>();
...
void SetExtraFactionRank(BaseExtraList& xDataList, TESFaction * faction, SInt8 rank)
{
	...
	if (!pData) {
		...
		pData = MemoryManager::Allocate<ExtraFactionChanges::FactionListData>();
		if (pData) {
