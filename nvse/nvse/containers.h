...
void* __fastcall Pool_Alloc_Buckets(UInt32 m_uiHashSize);
#else
...
void* Pool_Alloc_Buckets(UInt32 m_uiHashSize);
#endif
...
public:
	...
	LinkedList& operator=(const LinkedList &from)
	{
		...
		{
			newNode = AppendNew();
			RawAssign<T_Data>(newNode->m_val, pNode->m_val);
			pNode = pNode->next;
		}
		return *this;
	}
	...
	{
		Node *pNode = GetNthNode(index);
		return pNode ? &pNode->m_val : nullptr;
	}

	T_Data *Front() const
	{
		return head ? &head->m_val : nullptr;
	}

	T_Data *Back() const
	{
		return tail ? &tail->m_val : nullptr;
	}
	...
	public:
		...
		Data_Arg operator*() const {return pNode->m_val;}
		Data_Arg operator->() const {return pNode->m_val;}
		Data_Arg operator()() const {return pNode->m_val;}
		Data_Arg Get() const {return pNode->m_val;}
...
public:
	...
	public:
		Key_Arg Key() const {return pEntry->m_key.Get();}
...
template <typename T_Key, typename T_Data> class UnorderedMap
{
	...
	Bucket		*buckets;		// 00
	UInt32		m_uiHashSize;		// 04
	UInt32		numEntries;		// 08

	Bucket *End() const {return buckets + m_uiHashSize;}
	...
	__declspec(noinline) void ResizeTable(UInt32 newCount)
	{
		...
		while (pBucket != pEnd);
		Pool_Free(buckets, m_uiHashSize * sizeof(Bucket));
		buckets = newBuckets;
		m_uiHashSize = newCount + 1;
	}
	...
	Entry *FindEntry(Key_Arg key) const
	{
		if (numEntries)
		{
			...
			for (Entry *pEntry = buckets[hashVal & (m_uiHashSize - 1)].entries; pEntry; pEntry = pEntry->next)
	...
	bool InsertKey(Key_Arg key, T_Data **outData)
	{
		if (!buckets)
		{
			m_uiHashSize = AlignBucketCount(m_uiHashSize);
			buckets = (Bucket*)Pool_Alloc_Buckets(m_uiHashSize);
		}
		else if ((numEntries > m_uiHashSize) && (m_uiHashSize < MAP_MAX_BUCKET_COUNT))
			ResizeTable(m_uiHashSize << 1);
		UInt32 hashVal = HashKey<T_Key>(key);
		Bucket *pBucket = &buckets[hashVal & (m_uiHashSize - 1)];
...
public:
	UnorderedMap(UInt32 _m_uiHashSize = MAP_DEFAULT_BUCKET_COUNT) : buckets(nullptr), m_uiHashSize(_m_uiHashSize), numEntries(0) {}
	UnorderedMap(std::initializer_list<MappedPair<T_Key, T_Data>> inList) : buckets(nullptr), m_uiHashSize(inList.size()), numEntries(0) {InsertList(inList);}
	~UnorderedMap()
	{
		if (!buckets) return;
		Clear();
	...
	UInt32 BucketCount() const {return m_uiHashSize;}
	...
	void SetBucketCount(UInt32 newCount)
	{
		if (buckets)
		{
			...
			if ((m_uiHashSize != newCount) && (numEntries <= newCount))
				ResizeTable(newCount);
		}
		else m_uiHashSize = newCount;
	}

	float LoadFactor() const {return (float)numEntries / (float)m_uiHashSize;}
	...
	bool Erase(Key_Arg key)
	{
		if (numEntries)
		{
			...
			while (pEntry)
			{
				if (pEntry->m_key.Match(key, hashVal))
				{
					numEntries--;
	...
	public:
		...
		void Find(Key_Arg key)
		{
			...
			UInt32 hashVal = HashKey<T_Key>(key);
			bucket = &table->m_ppkHashTable[hashVal & (table->m_uiHashSize - 1)];
			entry = bucket->entries;
			while (entry)
			{
				if (entry->m_key.Match(key, hashVal))
					break;
				entry = entry->next;
			}
		}
		...
		Key_Arg Key() const {return entry->m_key.Get();}
	...
	__declspec(noinline) void ResizeTable(UInt32 newCount)
	{
		...
		do
		{
			...
			while (pEntry)
			{
				pTemp = pEntry;
				pEntry = pEntry->next;
				newBuckets[pTemp->m_key.GetHash() & newCount].Insert(pTemp);
			}
			++pBucket;
		}
		while (pBucket != pEnd);
...
public:
	UnorderedSet(UInt32 _m_uiHashSize = MAP_DEFAULT_BUCKET_COUNT) : buckets(nullptr), m_uiHashSize(_m_uiHashSize), numEntries(0) {}
	UnorderedSet(std::initializer_list<T_Key> inList) : buckets(nullptr), m_uiHashSize(inList.size()), numEntries(0) {InsertList(inList);}
	~UnorderedSet()
	{
		if (!buckets) return;
		Clear();
	...
	bool Insert(Key_Arg key)
	{
		...
		Bucket *pBucket = &buckets[hashVal & (m_uiHashSize - 1)];
		for (Entry *pEntry = pBucket->entries; pEntry; pEntry = pEntry->next)
			if (pEntry->m_key.Match(key, hashVal)) return false;
		numEntries++;
		Entry *newEntry = ALLOC_NODE(Entry);
		newEntry->m_key.Set(key, hashVal);
		pBucket->Insert(newEntry);
		return true;
	}
	...
	bool HasKey(Key_Arg key) const
	{
		if (numEntries)
		{
			UInt32 hashVal = HashKey<T_Key>(key);
			for (Entry *pEntry = buckets[hashVal & (m_uiHashSize - 1)].entries; pEntry; pEntry = pEntry->next)
				if (pEntry->m_key.Match(key, hashVal)) return true;
		}
		return false;
	}
	...
	public:
		...
		Key_Arg operator*() const {return entry->m_key.Get();}
		Key_Arg operator->() const {return entry->m_key.Get();}
		Key_Arg operator()() const {return entry->m_key.Get();}
		Key_Arg Key() const {return entry->m_key.Get();}
		bool End() const {return !entry;}
