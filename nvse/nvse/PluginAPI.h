...
struct NVSEArrayVarInterface
{
	...
	public:
		...
		void Reset() { if (type == kType_String) { MemoryManager::Deallocate(str); } type = kType_Invalid; str = NULL; }
		~Element() { Reset(); }
...
struct PluginScriptToken
{
	...
	//To properly destroy it, MemoryManager::Deallocate must be called.
