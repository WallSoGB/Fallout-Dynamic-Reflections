...
{
	if (text)
		return GetDeclaredVariableType(varInfo->name.pString, text, this);
	else
	{
...
{
	auto *scriptData = static_cast<UInt8 *>(m_val);
...
Script::RefVariable *ScriptBuffer::ResolveRef(const char *refName, Script *script)
{
	...
	auto *foundRef = refVars.FindFirst([&](Script::RefVariable *cur) { return cur->name.pString && !_stricmp(cur->name.pString, refName); });
	if (foundRef)
		newRef = foundRef;
	// not in list
...
{
	UInt32 idx = 0;
	for (Script::RefList::_Node *curEntry = refVars.Head(); curEntry && curEntry->m_item; curEntry = curEntry->m_pkNext)
	{
		idx++;
		if (ref == curEntry->m_item)
			break;
	}
...
Script::VariableType ScriptBuffer::GetVariableType(VariableInfo* varInfo, Script::RefVariable* refVar, Script* script)
{
	...
	if (refVar)
	{
		if (refVar->form)
		{
			...
			case kFormType_TESObjectREFR:
			{
				...
				scriptable = DYNAMIC_CAST(refr->pObjectReference, TESForm, TESScriptableForm);
				break;
			}
	...
	return GetDeclaredVariableType(varInfo->name.pString, scrText, script);
}
...
public:
	...
	bool Accept(VariableInfo *varInfo)
	{
		...
		if (!StrCompare(m_varName, varInfo->name.pString))
			return true;
		else
			return false;
	}
...
VariableInfo *Script::GetVariableByName(const char *varName)
{
	...
	do
	{
		auto *varInfo = varIter->m_item;
		if (varInfo && !StrCompare(varName, varInfo->name.pString))
			return varInfo;
	} while (varIter = varIter->m_pkNext);
	return NULL;
}
...
{
	for (auto *entry = varList.Head(); entry; entry = entry->m_pkNext)
		if (entry->m_item && entry->m_item->idx == idx)
			return entry->m_item;
...
{
	auto *var = MemoryManager::Allocate<RefVariable>();
...
{
	return FindFirst([&](const VariableInfo *v) { return StrCompare(v->name.pString, varName) == 0; });
}
