...
TESForm *TESForm::TryGetREFRParent(void)
{
	...
	if (result)
	{
		...
		if (refr && refr->pObjectReference)
			result = refr->pObjectReference;
	}
	return result;
}
...
TESFullName *TESForm::GetFullName() const
{
	if (typeID == kFormType_TESObjectCELL) // some exterior cells inherit name of parent worldspace
	{
		...
		TESFullName* fullName = &cell->name;
		if ((!fullName->strFullName.pString || !fullName->strFullName.usLen) && cell->pWorldSpace)
			return &cell->pWorldSpace->fullName;
		return fullName;
	}
	const TESForm *baseForm = IsReference() ? ((TESObjectREFR *)this)->pObjectReference : this;
...
const char *TESForm::GetTheName()
{
	...
	if (fullName)
		return fullName->strFullName.CStr();
	return "";
}

std::string TESForm::GetStringRepresentation() const
{
	return FormatString(R"([id: %X, edid: "%s", name: "%s"])", refID, GetName() ? GetName() : "", GetFullName() ? GetFullName()->strFullName.CStr() : "<no name>");
}
...
{
	partMask = (mask & TESBipedModelForm::eSlot_Full);
}
...
void TESBipedModelForm::SetPath(const char *newPath, UInt32 whichPath, bool bFemalePath)
{
	...
	{
	case ePath_Biped:
		toSet = &bipedModel[bFemalePath ? 1 : 0].cModel;
		break;
	case ePath_Ground:
		toSet = &groundModel[bFemalePath ? 1 : 0].cModel;
		break;
	case ePath_Icon:
		toSet = &icon[bFemalePath ? 1 : 0].TextureName;
		break;
	}
...
const char *TESBipedModelForm::GetPath(UInt32 whichPath, bool bFemalePath)
{
	...
	{
	case ePath_Biped:
		pathStr = &bipedModel[bFemalePath ? 1 : 0].cModel;
		break;
	case ePath_Ground:
		pathStr = &groundModel[bFemalePath ? 1 : 0].cModel;
		break;
	case ePath_Icon:
		pathStr = &icon[bFemalePath ? 1 : 0].TextureName;
		break;
	}
...
SInt8 TESActorBaseData::GetFactionRank(TESFaction *faction)
{
	...
	{
		FactionListData *m_val = iter.Get();
		if (m_val && (m_val->faction == faction))
			return m_val->rank;
	}
...
{
	TargetData *m_val = MemoryManager::Allocate<TargetData>();

	// fill out with same defaults as editor uses
	m_val->count = 0;
	m_val->target.objectCode = TESPackage::kObjectType_Activators;
	m_val->targetType = TESPackage::kTargetType_TypeCode;
...
{
	LocationData *m_val = MemoryManager::Allocate<LocationData>();

	m_val->locationType = kPackLocation_CurrentLocation;
	m_val->object.form = NULL;
	m_val->radius = 0;
...
{
	return pkHead.GetNthItem(whichItem);
}

const char *EffectItemList::GetNthEIName(UInt32 whichEffect) const
{
	EffectItem *effItem = pkHead.GetNthItem(whichEffect);
	if (effItem->setting)
