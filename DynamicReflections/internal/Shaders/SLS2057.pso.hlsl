...
#define	envreflect(i, n)	((2 * dot(i, n)) * (i)) - ((n) * dot(i, i))
    ...
    cubeMap.xyz = float3(IN.texcoord_1.w, IN.texcoord_2.w, IN.texcoord_3.w);

    r0.z = dot(q1.xyz, IN.texcoord_3.xyz);
    r0.y = dot(q1.xyz, IN.texcoord_2.xyz);
    r0.x = dot(q1.xyz, IN.texcoord_1.xyz);

    cubeCoord.rgb = envreflect(normalize(r0.xyz), normalize(cubeMap.xyz));
    ...
    q4.xyz = (cubeMap.rgb * (lerp(normals.a, envMask.x, EnvToggles.w) * EnvToggles.z)) * AmbientColor.rgba;
